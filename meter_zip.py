...
def gzip_compress(outFile, buf):
	with gzip.open(outFile, 'wb') as f:
		f.write(bytes(buf, encoding="utf8"))
	...
	for xlr in os.listdir(CONFIG_DIR):
		...
		for i in range(DATA_ROW_START_INDEX, sh.nrows):
			...
			for j in range(0, len(rows)):
				...
				row.append(value)
			jtxt.append(row)
		gzip_compress(os.path.join(OUTPUT_DIR, output + ".data"), json.dumps(jtxt))
		# with open(os.path.join(CONFIG_DIR, xlr), "w") as f: 
		# 		f.write(json.dumps(conf, ensure_ascii=True, indent=4))

		# with open(os.path.join(OUTPUT_DIR, output + ".data"), "w") as f:
		# 	f.write(json.dumps(jtxt, ensure_ascii=True, indent=4))
